...
void start_countdown(int delay)
{
    int start_time = time(NULL) + delay;
    int old_count = 0;
    int count = delay;

	while(count != 0)
    {
        count = start_time - time(NULL);

        if (count != old_count)
        {
            old_count = count;
            cprint("\r[**] ", BRIGHT, CYAN);
            printf("Experiment begins in: %i", count);
            fflush(stdout);
        }
    }
    cprint("\r[OK] ", BRIGHT, GREEN);
    printf("Experiment now in progress\n");
